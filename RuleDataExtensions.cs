...
internal static class RuleDataExtensions
{
    ...
    /// }</code>
    /// </example>
    public static event EventHandler<Exception> RuleDataAccessException
    {
    ...
    public static string GetFieldValue(this Rule rule, string className, string columnName, bool throwOnError = false, string defaultValue = null, int? rowNumber = null)
    {
        try
        {
            ...
            {
                if (null == rule.Data)
                    throw new NullReferenceException("Rule data is empty!");

                var ruleTable = rule.GetMultiRowRuleTable(className, throwOnError);

                if (!(ruleTable?.Columns?.Contains(columnName) ?? false))
                    throw new RuleDataFieldException(rule.GetName(), className, columnName, $"Rule data table {className} does not contain the required column '{columnName}'.");
            ...
            {
                if (!rule.Data.Fields.OfType<DataField>().Any(df => df.ClassName.Equals(className, StringComparison.OrdinalIgnoreCase) && df.FieldName.Equals(columnName, StringComparison.OrdinalIgnoreCase)) && throwOnError)
                    throw new ArgumentException($"Rule data does not contain the required class '{className}', or the class does not contain the required field '{columnName}'.");
